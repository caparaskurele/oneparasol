...
export default async function ArticlePage({
  ...
  const categoryName =
    CATEGORIES_EXCEL.find((c) => c.id === params.category)?.name ||
    params.category;
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-12">
      ...
      <nav className="mb-6 text-sm">
        <Link
          href="/knowledge_hub/career-growth-grid/data-scientist/excel"
          className="text-green-600 dark:text-green-400 hover:underline"
        >
          ← Back to Excel Learning
        </Link>
      ...
      {/* Featured Image */}
      {article.image && (
        <div className="mb-8 h-96 relative rounded-lg overflow-hidden bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900 dark:to-emerald-900">
      ...
      {article.resources && article.resources.length > 0 && (
        <div className="mb-12 p-6 bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 rounded-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {article.resources.map((resource, idx) => (
              <a
                ...
                className="p-4 bg-white dark:bg-green-800 rounded-lg border border-green-300 dark:border-green-600 hover:shadow-lg transition-shadow"
              >
      ...
      {/* Article Content */}
      <div className="prose dark:prose-invert prose-green max-w-none mb-12">
        <ReactMarkdown
          ...
          components={{
            ...
            code: ({ node, inline, children, ...props }: any) =>
              inline ? (
                <code
                  className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded text-green-600 dark:text-green-400 font-mono text-sm"
                  {...props}
                >
                  {children}
                </code>
            ...
            blockquote: ({ node, ...props }) => (
              <blockquote
                className="border-l-4 border-green-600 pl-4 py-2 text-gray-600 dark:text-gray-400 italic mb-4"
                {...props}
              />
            ),
          }}
